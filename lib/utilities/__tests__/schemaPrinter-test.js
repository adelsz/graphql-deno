...
import { assertObjectType, GraphQLList, GraphQLNonNull, GraphQLScalarType, GraphQLObjectType, GraphQLInterfaceType, GraphQLUnionType, GraphQLEnumType, GraphQLInputObjectType } from '../../type/definition.js';
import { buildSchema } from '../buildASTSchema.js';
import { printSchema, printIntrospectionSchema } from '../printSchema.js';

function printForTest(schema) {
  const schemaText = printSchema(schema); // keep printSchema and buildSchema in sync
...
describe('Type System Printer', () => {
  ...
  it('Prints String Field With String Arg With Default', () => {
    ...
    expect(output).to.equal( // $FlowFixMe
    dedent(String.raw`
        type Query {
  ...
  it('One-line prints a short description', () => {
    ...
    expect(recreatedField).to.include({
      description
    });
  });
