...
import { assertEnumType, GraphQLObjectType, GraphQLEnumType } from '../../type/definition.js';
import { GraphQLInt, GraphQLFloat, GraphQLString, GraphQLBoolean, GraphQLID } from '../../type/scalars.js';
...
describe('Type System: build schema from introspection', () => {
  ...
  it('builds a schema without the query type', () => {
    ...
    const introspection = introspectionFromSchema(schema); // $DisableFlowOnNegativeTest
  ...
  it('uses built-in scalars when possible', () => {
    ...
    const clientSchema = buildClientSchema(introspection); // Built-ins are used
    ...
    expect(clientSchema.getType('ID')).to.equal(GraphQLID); // Custom are built
  ...
  it('builds a schema with an enum', () => {
    ...
    expect(secondIntrospection).to.deep.equal(introspection); // It's also an Enum type on the client.

    const clientFoodEnum = assertEnumType(clientSchema.getType('Food')); // Client types do not get server-only values, so `value` mirrors `name`,
  ...
  describe('throws when given invalid introspection', () => {
    ...
    it('throws when introspection is missing __schema property', () => {
      ...
      expect(() => buildClientSchema(null)).to.throw('Invalid or incomplete introspection result. Ensure that you are passing "data" property of introspection response and no "errors" was returned alongside: null.'); // $DisableFlowOnNegativeTest

      expect(() => buildClientSchema({})).to.throw('Invalid or incomplete introspection result. Ensure that you are passing "data" property of introspection response and no "errors" was returned alongside: {}.');
    });
    it('throws when referenced unknown type', () => {
      const introspection = introspectionFromSchema(dummySchema); // $DisableFlowOnNegativeTest

      introspection.__schema.types = introspection.__schema.types.filter(({
        name
      }) => name !== 'Query');
      expect(() => buildClientSchema(introspection)).to.throw('Invalid or incomplete schema, unknown type: Query. Ensure that a full introspection query is used in order to build a client schema.');
    });
    it('throws when missing definition for one of the standard scalars', () => {
      ...
      expect(() => buildClientSchema(introspection)).to.throw('Invalid or incomplete schema, unknown type: Float. Ensure that a full introspection query is used in order to build a client schema.');
    });
    it('throws when type reference is missing name', () => {
      const introspection = introspectionFromSchema(dummySchema);
      expect(introspection).to.have.nested.property('__schema.queryType.name'); // $DisableFlowOnNegativeTest

      delete introspection.__schema.queryType.name;
      expect(() => buildClientSchema(introspection)).to.throw('Unknown type reference: {}.');
    });
    it('throws when missing kind', () => {
      ...
      expect(queryTypeIntrospection).to.have.property('kind'); // $DisableFlowOnNegativeTest

      delete queryTypeIntrospection.kind;
      expect(() => buildClientSchema(introspection)).to.throw(/Invalid or incomplete introspection result. Ensure that a full introspection query is used in order to build a client schema: { name: "Query", .* }\./);
    });
    it('throws when missing interfaces', () => {
      ...
      expect(queryTypeIntrospection).to.have.property('interfaces'); // $DisableFlowOnNegativeTest

      delete queryTypeIntrospection.interfaces;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing interfaces: { kind: "OBJECT", name: "Query", .* }\./);
    });
    it('Legacy support for interfaces with null as interfaces field', () => {
      ...
      expect(someInterfaceIntrospection).to.have.property('interfaces'); // $DisableFlowOnNegativeTest
    ...
    it('throws when missing fields', () => {
      ...
      expect(queryTypeIntrospection).to.have.property('fields'); // $DisableFlowOnNegativeTest

      delete queryTypeIntrospection.fields;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing fields: { kind: "OBJECT", name: "Query", .* }\./);
    });
    it('throws when missing field args', () => {
      ...
      expect(queryTypeIntrospection).to.have.nested.property('fields[0].args'); // $DisableFlowOnNegativeTest

      delete queryTypeIntrospection.fields[0].args;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing field args: { name: "foo", .* }\./);
    });
    it('throws when output type is used as an arg type', () => {
      ...
      expect(queryTypeIntrospection).to.have.nested.property('fields[0].args[0].type.name', 'String'); // $DisableFlowOnNegativeTest

      queryTypeIntrospection.fields[0].args[0].type.name = 'SomeUnion';
      expect(() => buildClientSchema(introspection)).to.throw('Introspection must provide input type for arguments, but received: SomeUnion.');
    });
    it('throws when input type is used as a field type', () => {
      ...
      expect(queryTypeIntrospection).to.have.nested.property('fields[0].type.name', 'String'); // $DisableFlowOnNegativeTest

      queryTypeIntrospection.fields[0].type.name = 'SomeInputObject';
      expect(() => buildClientSchema(introspection)).to.throw('Introspection must provide output type for fields, but received: SomeInputObject.');
    });
    it('throws when missing possibleTypes', () => {
      ...
      expect(someUnionIntrospection).to.have.property('possibleTypes'); // $DisableFlowOnNegativeTest

      delete someUnionIntrospection.possibleTypes;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing possibleTypes: { kind: "UNION", name: "SomeUnion",.* }\./);
    });
    it('throws when missing enumValues', () => {
      ...
      expect(someEnumIntrospection).to.have.property('enumValues'); // $DisableFlowOnNegativeTest

      delete someEnumIntrospection.enumValues;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing enumValues: { kind: "ENUM", name: "SomeEnum", .* }\./);
    });
    it('throws when missing inputFields', () => {
      ...
      expect(someInputObjectIntrospection).to.have.property('inputFields'); // $DisableFlowOnNegativeTest

      delete someInputObjectIntrospection.inputFields;
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing inputFields: { kind: "INPUT_OBJECT", name: "SomeInputObject", .* }\./);
    });
    it('throws when missing directive locations', () => {
      ...
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing directive locations: { name: "SomeDirective", .* }\./);
    });
    it('throws when missing directive args', () => {
      ...
      expect(() => buildClientSchema(introspection)).to.throw(/Introspection result missing directive args: { name: "SomeDirective", .* }\./);
    });
  });
  describe('very deep decorators are not supported', () => {
    it('fails on very deep (> 7 levels) lists', () => {
      ...
      `);
      const introspection = introspectionFromSchema(schema);
      expect(() => buildClientSchema(introspection)).to.throw('Decorated type deeper than introspection query.');
    });
  ...
  describe('prevents infinite recursion on invalid introspection', () => {
    it('recursive interfaces', () => {
      ...
      expect(() => buildClientSchema(introspection)).to.throw('Expected Foo to be a GraphQL Interface type.');
    });
    it('recursive union', () => {
      const sdl = `
        type Query {
      ...
      expect(() => buildClientSchema(introspection)).to.throw('Expected Foo to be a GraphQL Object type.');
    });
  });
});