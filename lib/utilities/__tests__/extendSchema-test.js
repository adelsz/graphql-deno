...
import { assertDirective } from '../../type/directives.js';
import { GraphQLID, GraphQLInt, GraphQLFloat, GraphQLString, GraphQLBoolean } from '../../type/scalars.js';
import { assertObjectType, assertInputObjectType, assertEnumType, assertUnionType, assertInterfaceType, assertScalarType } from '../../type/definition.js';
...
function printSchemaChanges(schema, extendedSchema) {
  ...
  return print({
    kind: Kind.DOCUMENT,
    definitions: ast.definitions.filter(node => !schemaDefinitions.includes(print(node)))
  });
}
...
describe('extendSchema', () => {
  ...
  it('extends objects with standard type fields', () => {
    const schema = buildSchema('type Query'); // String and Boolean are always included through introspection types
  ...
  it('correctly assign AST nodes to new and extended types', () => {
    ...
    `);
    const extendedTwiceSchema = extendSchema(extendedSchema, secondExtensionAST);
    const extendedInOneGoSchema = extendSchema(schema, concatAST([firstExtensionAST, secondExtensionAST]));
    ...
    const someScalar = assertScalarType(extendedTwiceSchema.getType('SomeScalar'));
    const someInput = assertInputObjectType(extendedTwiceSchema.getType('SomeInput'));
    const someInterface = assertInterfaceType(extendedTwiceSchema.getType('SomeInterface'));
    const testInput = assertInputObjectType(extendedTwiceSchema.getType('TestInput'));
    ...
    const testInterface = assertInterfaceType(extendedTwiceSchema.getType('TestInterface'));
    const testDirective = assertDirective(extendedTwiceSchema.getDirective('test'));
    expect(testType).to.include({
      extensionASTNodes: undefined
    });
    expect(testEnum).to.include({
      extensionASTNodes: undefined
    });
    expect(testUnion).to.include({
      extensionASTNodes: undefined
    });
    expect(testInput).to.include({
      extensionASTNodes: undefined
    });
    expect(testInterface).to.include({
      extensionASTNodes: undefined
    });
  ...
  it('sets correct description using legacy comments', () => {
    ...
    expect(newDirective).to.include({
  ...
  it('Rejects invalid SDL', () => {
    ...
    expect(() => extendSchema(schema, extendAST)).to.throw('Unknown directive "@unknown".');
  });
  ...
  it('Throws on unknown types', () => {
    ...
    `);
    expect(() => extendSchema(schema, ast, {
      assumeValidSDL: true
    })).to.throw('Unknown type: "UnknownType".');
  });
  it('Rejects invalid AST', () => {
    const schema = new GraphQLSchema({}); // $DisableFlowOnNegativeTest

    expect(() => extendSchema(schema, null)).to.throw('Must provide valid Document AST'); // $DisableFlowOnNegativeTest

    expect(() => extendSchema(schema, {})).to.throw('Must provide valid Document AST');
  });
  it('does not allow replacing a default directive', () => {
    ...
    `);
    expect(() => extendSchema(schema, extendAST)).to.throw('Directive "@include" already exists in the schema. It cannot be redefined.');
  });
  it('does not allow replacing an existing enum value', () => {
    ...
    `);
    expect(() => extendSchema(schema, extendAST)).to.throw('Enum value "SomeEnum.ONE" already exists in the schema. It cannot also be defined in this type extension.');
  });
  describe('can add additional root operation types', () => {
    ...
    it('adds new root types via schema extension', () => {
      ...
      expect(mutationType).to.include({
        name: 'MutationRoot'
      });
    ...
    it('adds multiple new root types via schema extension', () => {
      ...
      expect(subscriptionType).to.include({
        name: 'Subscription'
      });
    });
