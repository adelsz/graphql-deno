...
import { findDeprecatedUsages } from '../findDeprecatedUsages.js';
describe('findDeprecatedUsages', () => {
  ...
  it('should report empty set for no deprecated usages', () => {
    const errors = findDeprecatedUsages(schema, parse('{ normalField(enumArg: [NORMAL_VALUE]) }'));
    expect(errors.length).to.equal(0);
  });
  it('should ignore unknown stuff', () => {
    const errors = findDeprecatedUsages(schema, parse(`
        {
  ...
  it('should report usage of deprecated fields', () => {
    const errors = findDeprecatedUsages(schema, parse('{ normalField, deprecatedField }'));
  ...
  it('should report usage of deprecated enums', () => {
    const errors = findDeprecatedUsages(schema, parse(`
      ...
      `));
    const errorMessages = errors.map(err => err.message);
    expect(errorMessages).to.deep.equal(['The enum value "EnumType.DEPRECATED_VALUE" is deprecated. Some enum reason.']);
  });
});