...
import { kitchenSinkQuery, kitchenSinkSDL } from '../../__fixtures__/index.js';
const ignoredTokens = [// UnicodeBOM ::
...
const nonPunctuatorTokens = ['name_token', // Name
'1', // IntValue
'3.14', // FloatValue
...
function expectStripped(docString) {
  return {
    toEqual(expected) {
      ...
      invariant(stripped === expected, dedent`
      ...
      invariant(stripped === strippedTwice, dedent`
...
describe('stripIgnoredCharacters', () => {
  it('asserts that a source was provided', () => {
    ...
    expect(() => stripIgnoredCharacters()).to.throw('Must provide string or Source. Received: undefined.');
  });
  it('asserts that a valid source was provided', () => {
    // $DisableFlowOnNegativeTest
    expect(() => stripIgnoredCharacters({})).to.throw('Must provide string or Source. Received: {}.');
  });
  ...
  it('strips ignored characters inside block strings', () => {
    function expectStrippedString(blockStr) {
      ...
      invariant(originalValue === strippedValue, dedent`
    ...
    expectStrippedString('"""\na\n b"""').toStayTheSame();
    expectStrippedString('"""\n a\n b"""').toEqual('"""a\nb"""');
    expectStrippedString('"""\na\n b\nc"""').toEqual('"""a\n b\nc"""'); // Testing with length >5 is taking exponentially more time. However it is
    ...
    for (let length = 1; length < maxCombinationLength; ++length) {
      ...
      for (let combination = 0; combination < numCombinations; ++combination) {
        ...
        invariant(testValue === strippedValue, dedent`
  ...
  it('strips kitchen sink SDL but maintains the exact same AST', () => {
    ...
    const sdlAST = parse(kitchenSinkSDL, {
      noLocation: true
    });
    const strippedAST = parse(strippedSDL, {
      noLocation: true
    });
