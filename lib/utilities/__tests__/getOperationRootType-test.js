...
describe('getOperationRootType', () => {
  ...
  it('Throws when query type not defined in schema', () => {
    ...
    const operationNode = getOperationNode(doc);
    expect(() => getOperationRootType(testSchema, operationNode)).to.throw('Schema does not define the required query root type.');
  });
  it('Throws when mutation type not defined in schema', () => {
    const testSchema = new GraphQLSchema({});
    const doc = parse('mutation { field }');
    const operationNode = getOperationNode(doc);
    expect(() => getOperationRootType(testSchema, operationNode)).to.throw('Schema is not configured for mutations.');
  });
  it('Throws when subscription type not defined in schema', () => {
    const testSchema = new GraphQLSchema({});
    const doc = parse('subscription { field }');
    const operationNode = getOperationNode(doc);
    expect(() => getOperationRootType(testSchema, operationNode)).to.throw('Schema is not configured for subscriptions.');
  });
  it('Throws when operation not a valid operation kind', () => {
    ...
    expect(() => getOperationRootType(testSchema, operationNode)).to.throw('Can only have query, mutation and subscription operations.');
  });
});