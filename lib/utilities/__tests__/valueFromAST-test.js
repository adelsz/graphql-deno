...
import { GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType, GraphQLList, GraphQLNonNull } from '../../type/definition.js';
...
describe('valueFromAST', () => {
  ...
  const nonNullBool = GraphQLNonNull(GraphQLBoolean); // [Boolean]

  const listOfBool = GraphQLList(GraphQLBoolean); // [Boolean!]

  const listOfNonNullBool = GraphQLList(nonNullBool); // [Boolean]!

  const nonNullListOfBool = GraphQLNonNull(listOfBool); // [Boolean!]!
  ...
  it('coerces non-null lists of values', () => {
    ...
    expectValueFrom('[true, false]', nonNullListOfBool).to.deep.equal([true, false]);
    expectValueFrom('[true, 123]', nonNullListOfBool).to.equal(undefined);
    expectValueFrom('[true, null]', nonNullListOfBool).to.deep.equal([true, null]);
  });
  it('coerces lists of non-null values', () => {
    ...
    expectValueFrom('[true, false]', listOfNonNullBool).to.deep.equal([true, false]);
  ...
  it('coerces non-null lists of non-null values', () => {
    ...
    expectValueFrom('[true, false]', nonNullListOfNonNullBool).to.deep.equal([true, false]);
  ...
  it('accepts variable values assuming already coerced', () => {
    ...
    expectValueFrom('$var', GraphQLBoolean, {
      var: true
    }).to.equal(true);
    expectValueFrom('$var', GraphQLBoolean, {
      var: null
    }).to.equal(null);
    expectValueFrom('$var', nonNullBool, {
      var: null
    }).to.equal(undefined);
  });
  it('asserts variables are provided as items in lists', () => {
    ...
    expectValueFrom('[ $foo ]', listOfNonNullBool, {
      foo: true
    }).to.deep.equal([true]); // Note: variables are expected to have already been coerced, so we
  ...
  it('omits input object fields for unprovided variables', () => {
    expectValueFrom('{ int: $foo, bool: $foo, requiredBool: true }', testInputObj, {}).to.deep.equal({
      int: 42,
      requiredBool: true
    });
