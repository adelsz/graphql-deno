...
describe('coerceInputValue', () => {
  ...
  describe('for GraphQLInputObject', () => {
    ...
    it('returns no error for a valid input', () => {
      ...
      expectValue(result).to.deep.equal({
        foo: 123
      });
    });
  ...
  describe('for GraphQLInputObject with default value', () => {
    const TestInputObject = defaultValue => new GraphQLInputObjectType({
      name: 'TestInputObject',
      fields: {
        foo: {
  ...
  describe('with default onError', () => {
    it('throw error without path', () => {
      expect(() => coerceInputValue(null, GraphQLNonNull(GraphQLInt))).to.throw('Invalid value null: Expected non-nullable type "Int!" not to be null.');
    });
    it('throw error with path', () => {
      expect(() => coerceInputValue([null], GraphQLList(GraphQLNonNull(GraphQLInt)))).to.throw('Invalid value null at "value[0]": Expected non-nullable type "Int!" not to be null.');
    });
  });
});