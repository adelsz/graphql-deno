...
import { __Schema } from '../../type/introspection.js';
import { assertDirective, GraphQLSkipDirective, GraphQLIncludeDirective, GraphQLDeprecatedDirective } from '../../type/directives.js';
...
import { printType, printSchema } from '../printSchema.js';
...
describe('Schema Builder', () => {
  ...
  it('Simple type', () => {
    ...
    `;
    expect(cycleSDL(sdl)).to.equal(sdl);
    const schema = buildSchema(sdl); // Built-ins are used
  ...
  it('Empty interface', () => {
    ...
    expect(definition.kind === 'InterfaceTypeDefinition' && definition.interfaces).to.deep.equal([], 'The interfaces property must be an empty array.');
    expect(cycleSDL(sdl)).to.equal(sdl);
  });
  ...
  it('Correctly assign AST nodes', () => {
    ...
    expect([schema.astNode, query.astNode, testInput.astNode, testEnum.astNode, testUnion.astNode, testInterface.astNode, testType.astNode, testScalar.astNode, testDirective.astNode]).to.be.deep.equal(ast.definitions);
  ...
  it('Throws on unknown types', () => {
    ...
    `;
    expect(() => buildSchema(sdl, {
      assumeValidSDL: true
    })).to.throw('Unknown type: "UnknownType".');
  });
  it('Rejects invalid AST', () => {
    // $DisableFlowOnNegativeTest
    expect(() => buildASTSchema(null)).to.throw('Must provide valid Document AST'); // $DisableFlowOnNegativeTest

    expect(() => buildASTSchema({})).to.throw('Must provide valid Document AST');
  });
});