...
import { GraphQLList, GraphQLNonNull, GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType } from '../../type/definition.js';
...
describe('astFromValue', () => {
  ...
  it('converts Int values to Int ASTs', () => {
    ...
    expect(() => astFromValue(123.5, GraphQLInt)).to.throw('Int cannot represent non-integer value: 123.5'); // Note: outside the bounds of 32bit signed int.

    expect(() => astFromValue(1e40, GraphQLInt)).to.throw('Int cannot represent non 32-bit signed integer value: 1e+40');
    expect(() => astFromValue(NaN, GraphQLInt)).to.throw('Int cannot represent non-integer value: NaN');
  });
  ...
  it('converts ID values to Int/String ASTs', () => {
    ...
    expect(() => astFromValue(false, GraphQLID)).to.throw('ID cannot represent value: false');
  ...
  it('converts using serialize from a custom scalar type', () => {
    ...
    expect(() => astFromValue(NaN, passthroughScalar)).to.throw('Cannot convert value to AST: NaN.');
    expect(() => astFromValue(Infinity, passthroughScalar)).to.throw('Cannot convert value to AST: Infinity.');
    ...
    expect(() => astFromValue('value', returnCustomClassScalar)).to.throw('Cannot convert value to AST: {}.');
  });
  ...
  it('converts string values to Enum ASTs if possible', () => {
    ...
    expect(() => astFromValue('hello', myEnum)).to.throw('Enum "MyEnum" cannot represent value: "hello"'); // Note: Not a valid enum value

    expect(() => astFromValue('UNKNOWN_VALUE', myEnum)).to.throw('Enum "MyEnum" cannot represent value: "UNKNOWN_VALUE"');
  });
  it('converts array values to List ASTs', () => {
    expect(astFromValue(['FOO', 'BAR'], GraphQLList(GraphQLString))).to.deep.equal({
      kind: 'ListValue',
      values: [{
        kind: 'StringValue',
        value: 'FOO'
    ...
    expect(astFromValue(['HELLO', 'GOODBYE'], GraphQLList(myEnum))).to.deep.equal({
      kind: 'ListValue',
      values: [{
        kind: 'EnumValue',
        value: 'HELLO'
  ...
  it('skip invalid list items', () => {
    const ast = astFromValue(['FOO', null, 'BAR'], GraphQLList(GraphQLNonNull(GraphQLString)));
