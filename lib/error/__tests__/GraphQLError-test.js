...
import { GraphQLError, printError } from '../GraphQLError.js';
...
describe('GraphQLError', () => {
  ...
  it('uses the stack of an original error', () => {
    ...
    const e = new GraphQLError('msg', undefined, undefined, undefined, undefined, original);
    expect(e).to.include({
      name: 'GraphQLError',
      message: 'msg',
  ...
  it('serializes to include message and locations', () => {
    const e = new GraphQLError('msg', fieldNode);
    expect(JSON.stringify(e)).to.equal('{"message":"msg","locations":[{"line":2,"column":3}]}');
  });
  it('serializes to include path', () => {
    ...
    expect(JSON.stringify(e)).to.equal('{"message":"msg","path":["path",3,"to","field"]}');
  });
});
describe('printError', () => {
  ...
  it('prints an error using node without location', () => {
    const error = new GraphQLError('Error attached to node without location', parse('{ foo }', {
      noLocation: true
    }));
    expect(printError(error)).to.equal('Error attached to node without location');
  });
  it('prints an error with nodes from different sources', () => {
    const docA = parse(new Source(dedent`
          type Foo {
            field: String
          }
        `, 'SourceA'));
    ...
    const docB = parse(new Source(dedent`
          type Foo {
            field: Int
          }
        `, 'SourceB'));
    ...
    const error = new GraphQLError('Example error with two nodes', [fieldA.type, fieldB.type]);
