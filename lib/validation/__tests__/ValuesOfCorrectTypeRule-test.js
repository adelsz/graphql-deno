...
import { GraphQLScalarType, GraphQLObjectType } from '../../type/definition.js';
...
function expectErrorsWithSchema(schema, queryStr) {
  return expectValidationErrorsWithSchema(schema, ValuesOfCorrectTypeRule, queryStr);
}
...
describe('Validate: Values of correct type', () => {
  ...
  describe('Invalid input object value', () => {
    ...
    it('reports error for custom scalar that returns undefined', () => {
      ...
      const expectedErrors = expectErrorsWithSchema(schema, `
        {
          invalidArg(arg: 123)
        }
      `);
