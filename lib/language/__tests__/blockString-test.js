...
import { dedentBlockStringValue, getBlockStringIndentation, printBlockString } from '../blockString.js';
...
describe('dedentBlockStringValue', () => {
  it('removes uniform indentation from a string', () => {
    const rawValue = joinLines('', '    Hello,', '      World!', '', '    Yours,', '      GraphQL.');
...
describe('printBlockString', () => {
  ...
  it('correctly prints string with a first line indentation', () => {
    ...
    expect(printBlockString(str)).to.equal(joinLines('"""', '    first  ', '  line     ', 'indentation', '     string', '"""'));
  });
});