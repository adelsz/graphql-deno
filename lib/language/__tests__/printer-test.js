...
describe('Printer: Query document', () => {
  ...
  it('produces helpful error messages', () => {
    ...
    expect(() => print(badAST)).to.throw('Invalid AST Node: { random: "Data" }.');
  });
  ...
  it('prints query with variable directives', () => {
    const queryASTWithVariableDirective = parse('query ($foo: TestType = {a: 123} @testDirective(if: true) @test) { id }');
  ...
  it('Experimental: prints fragment with variable directives', () => {
    const queryASTWithVariableDirective = parse('fragment Foo($foo: TestType @test) on TestType @testDirective { id }', {
  ...
  it('prints kitchen sink', () => {
    ...
    expect(printed).to.equal( // $FlowFixMe
    dedent(String.raw`
