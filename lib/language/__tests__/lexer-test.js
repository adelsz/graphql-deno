import { inspect as nodeInspect } from 'util.js';
...
describe('Lexer', () => {
  ...
  it('can be JSON.stringified, util.inspected or jsutils.inspect', () => {
    ...
    expect(JSON.stringify(token)).to.equal('{"kind":"Name","value":"foo","line":1,"column":1}');
    expect(nodeInspect(token)).to.equal("{ kind: 'Name', value: 'foo', line: 1, column: 1 }");
    expect(inspect(token)).to.equal('{ kind: "Name", value: "foo", line: 1, column: 1 }');
  });
...
`)).to.contain({
  ...
  it('updates line numbers in error for file context', () => {
    ...
    try {
      const str = ['', '', '     ?', ''].join('\n');
      const source = new Source(str, 'foo.js', {
        line: 11,
        column: 12
      });
  ...
  it('updates column numbers in error for file context', () => {
    ...
    try {
      const source = new Source('?', 'foo.js', {
        line: 1,
        column: 5
      });
      new Lexer(source).advance();
  ...
  it('lex reports useful information for dashes in names', () => {
    ...
    expect(() => lexer.advance()).throw(GraphQLError).that.deep.include({
      message: 'Syntax Error: Invalid number, expected digit but got: "b".',
      locations: [{
        line: 1,
        column: 3
  ...
  it('produces double linked list of tokens, including comments', () => {
    ...
    expect(tokens.map(tok => tok.kind)).to.deep.equal([TokenKind.SOF, TokenKind.BRACE_L, TokenKind.COMMENT, TokenKind.NAME, TokenKind.BRACE_R, TokenKind.EOF]);
  });
});
