...
describe('isCollection', () => {
  it('should return `true` for collections', () => {
    ...
    expect(isCollection(new Int8Array(1))).to.equal(true); // eslint-disable-next-line no-new-wrappers
    ...
    expect(isCollection(iterator)).to.equal(true); // istanbul ignore next
  ...
  it('should return `false` for non-collections', () => {
    ...
    expect(isCollection(0)).to.equal(false);
    expect(isCollection(NaN)).to.equal(false); // eslint-disable-next-line no-new-wrappers

    expect(isCollection(new Number(123))).to.equal(false);
    expect(isCollection(true)).to.equal(false);
    expect(isCollection(false)).to.equal(false); // eslint-disable-next-line no-new-wrappers
