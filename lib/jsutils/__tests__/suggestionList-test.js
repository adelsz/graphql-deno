...
describe('suggestionList', () => {
  ...
  it('Returns options with different case', () => {
    ...
    expectSuggestions('verylongstring', ['VERYLONGSTRING']).to.deep.equal(['VERYLONGSTRING']);
    expectSuggestions('VERYLONGSTRING', ['verylongstring']).to.deep.equal(['verylongstring']);
    expectSuggestions('VERYLONGSTRING', ['VeryLongString']).to.deep.equal(['VeryLongString']);
  });
  ...
  it('Returns options sorted based on lexical distance', () => {
    expectSuggestions('abc', ['a', 'ab', 'abc']).to.deep.equal(['abc', 'ab', 'a']);
  });
  it('Returns options with the same lexical distance sorted lexicographically', () => {
    expectSuggestions('a', ['az', 'ax', 'ay']).to.deep.equal(['ax', 'ay', 'az']);
  });
});