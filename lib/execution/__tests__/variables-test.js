...
import { GraphQLList, GraphQLNonNull, GraphQLScalarType, GraphQLObjectType, GraphQLInputObjectType, GraphQLEnumType } from '../../type/definition.js';
...
const TestType = new GraphQLObjectType({
  name: 'TestType',
  fields: {
    fieldWithEnumInput: fieldWithInputArg({
      type: TestEnum
    }),
...
describe('Execute: Handles inputs', () => {
  describe('Handles objects and nullability', () => {
    ...
    describe('using variables', () => {
      ...
      it('errors on deep nested errors and with many errors', () => {
        ...
        `;
        const result = executeQuery(nestedDoc, {
          input: {
            na: {
              a: 'foo'
            }
  ...
  describe('getVariableValues: limit maximum number of coercion errors', () => {
    ...
    it('when maxErrors is less than number of errors', () => {
      ...
      expect(result).to.deep.equal({
        errors: [invalidValueError(0, 0), invalidValueError(1, 1), {
