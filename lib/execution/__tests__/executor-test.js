...
import { GraphQLInt, GraphQLBoolean, GraphQLString } from '../../type/scalars.js';
import { GraphQLList, GraphQLNonNull, GraphQLScalarType, GraphQLInterfaceType, GraphQLObjectType } from '../../type/definition.js';
...
describe('Execute: Handles basic execution tasks', () => {
  ...
  it('throws if no schema is provided', () => {
    const document = parse('{ field }'); // $DisableFlowOnNegativeTest
  ...
  it('throws on invalid variables', () => {
    ...
    `);
    const variableValues = '{ "a": 1 }'; // $DisableFlowOnNegativeTest

    expect(() => execute({
      schema,
      document,
      variableValues
    })).to.throw('Variables must be provided as an Object where each property is a variable value. Perhaps look to see if an unparsed JSON string was provided.');
  });
  ...
  it('correctly threads arguments', () => {
    ...
    expect(resolvedArgs).to.deep.equal({
      numArg: 123,
      stringArg: 'foo'
    });
  });
