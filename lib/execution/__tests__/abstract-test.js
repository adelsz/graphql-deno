...
import { GraphQLString, GraphQLBoolean } from '../../type/scalars.js';
import { GraphQLList, GraphQLObjectType, GraphQLInterfaceType, GraphQLUnionType } from '../../type/definition.js';
...
describe('Execute: Handles execution of abstract types', () => {
  ...
  it('returning invalid value from resolveType yields useful error', () => {
    ...
    expect(result).to.deep.equal({
      ...
      errors: [{
        message: 'Abstract type "FooInterface" must resolve to an Object type at runtime for field "Query.foo" with value "dummy", received "[]". Either the "FooInterface" type should provide a "resolveType" function or each possible type should provide an "isTypeOf" function.',
        locations: [{
          line: 1,
          column: 3
        }],
  ...
  it('missing both resolveType and isTypeOf yields useful error', () => {
    ...
    expect(result).to.deep.equal({
      ...
      errors: [{
        message: 'Abstract type "FooInterface" must resolve to an Object type at runtime for field "Query.foo" with value "dummy", received "undefined". Either the "FooInterface" type should provide a "resolveType" function or each possible type should provide an "isTypeOf" function.',
        locations: [{
          line: 1,
          column: 3
        }],
