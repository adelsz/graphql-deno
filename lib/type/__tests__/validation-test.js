...
import { validateSchema, assertValidSchema } from '../validate.js';
import { GraphQLDirective, assertDirective } from '../directives.js';
import { GraphQLList, GraphQLNonNull, GraphQLObjectType, GraphQLInterfaceType, GraphQLUnionType, GraphQLEnumType, GraphQLInputObjectType, assertScalarType, assertInterfaceType, assertObjectType, assertUnionType, assertEnumType, assertInputObjectType } from '../definition.js';
...
const SomeInterfaceType = assertInterfaceType(SomeSchema.getType('SomeInterface'));
...
const SomeInputObjectType = assertInputObjectType(SomeSchema.getType('SomeInputObject'));
const SomeDirective = assertDirective(SomeSchema.getDirective('SomeDirective'));

function withModifiers(type) {
  return [type, GraphQLList(type), GraphQLNonNull(type), GraphQLNonNull(GraphQLList(type))];
}

const outputTypes = [...withModifiers(GraphQLString), ...withModifiers(SomeScalarType), ...withModifiers(SomeEnumType), ...withModifiers(SomeObjectType), ...withModifiers(SomeUnionType), ...withModifiers(SomeInterfaceType)];
const notOutputTypes = [...withModifiers(SomeInputObjectType)];
const inputTypes = [...withModifiers(GraphQLString), ...withModifiers(SomeScalarType), ...withModifiers(SomeEnumType), ...withModifiers(SomeInputObjectType)];
const notInputTypes = [...withModifiers(SomeObjectType), ...withModifiers(SomeUnionType), ...withModifiers(SomeInterfaceType)];
...
describe('Type System: Objects must have fields', () => {
  ...
  it('rejects an Object type with missing fields', () => {
    ...
    const manualSchema = schemaWithFieldType(new GraphQLObjectType({
    ...
    const manualSchema2 = schemaWithFieldType(new GraphQLObjectType({
      name: 'IncompleteObject',
  ...
  it('rejects an Object type with incorrectly named fields', () => {
    const schema = schemaWithFieldType(new GraphQLObjectType({
      name: 'SomeObject',
      fields: {
...
describe('Type System: Union types must be valid', () => {
  ...
  it('rejects a Union type with non-Object members types', () => {
    ...
    const badUnionMemberTypes = [GraphQLString, new GraphQLNonNull(SomeObjectType), new GraphQLList(SomeObjectType), SomeInterfaceType, SomeUnionType, SomeEnumType, SomeInputObjectType];
    ...
    for (const memberType of badUnionMemberTypes) {
      ...
      expect(validateSchema(badSchema)).to.deep.equal([{
        message: 'Union type BadUnion can only include Object types, ' + `it cannot include ${inspect(memberType)}.`
      }]);
    }
  });
});
...
describe('Type System: Objects can only implement unique interfaces', () => {
  ...
  it('rejects an Object implementing the same interface twice due to extension', () => {
    ...
    `);
    const extendedSchema = extendSchema(schema, parse('extend type AnotherObject implements AnotherInterface'));
...
describe('Type System: Interface extensions should be valid', () => {
  it('rejects an Object implementing the extended interface due to missing field', () => {
    ...
    `);
    const extendedSchema = extendSchema(schema, parse(`
...
describe('Type System: Arguments must have input types', () => {
  ...
  it('rejects a non-type value as a field arg type', () => {
    ...
    expect(validateSchema(schema)).to.deep.equal([{
      message: 'The type of @BadDirective(badArg:) must be Input Type but got: [function Number].'
    }, {
