...
import { GraphQLDirective, GraphQLSkipDirective, GraphQLIncludeDirective, GraphQLDeprecatedDirective, assertDirective, isDirective, isSpecifiedDirective } from '../directives.js';
import { GraphQLID, GraphQLInt, GraphQLFloat, GraphQLString, GraphQLBoolean, isSpecifiedScalarType } from '../scalars.js';
import { GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType, GraphQLInterfaceType, GraphQLObjectType, GraphQLUnionType, GraphQLList, GraphQLNonNull, isType, isScalarType, isObjectType, isInterfaceType, isUnionType, isEnumType, isInputObjectType, isListType, isNonNullType, isInputType, isOutputType, isLeafType, isCompositeType, isAbstractType, isWrappingType, isNullableType, isNamedType, isRequiredArgument, isRequiredInputField, assertType, assertScalarType, assertObjectType, assertInterfaceType, assertUnionType, assertEnumType, assertInputObjectType, assertListType, assertNonNullType, assertInputType, assertOutputType, assertLeafType, assertCompositeType, assertAbstractType, assertWrappingType, assertNullableType, assertNamedType, getNullableType, getNamedType } from '../definition.js';
...
describe('Type predicates', () => {
  ...
  describe('isInputObjectType', () => {
    ...
    it('returns false for wrapped input object type', () => {
      ...
      expect(() => assertInputObjectType(GraphQLList(InputObjectType))).to.throw();
    });
  ...
  describe('isListType', () => {
    ...
    it('returns false for a non-list wrapped type', () => {
      ...
      expect(() => assertListType(GraphQLNonNull(GraphQLList(ObjectType)))).to.throw();
    });
  });
  describe('isNonNullType', () => {
    it('returns true for a non-null wrapped type', () => {
      ...
      expect(() => assertNonNullType(GraphQLNonNull(ObjectType))).to.not.throw();
    });
    it('returns false for an unwrapped type', () => {
    ...
    it('returns false for a not non-null wrapped type', () => {
      ...
      expect(() => assertNonNullType(GraphQLList(GraphQLNonNull(ObjectType)))).to.throw();
    });
  });
  ...
  describe('isCompositeType', () => {
    ...
    it('returns false for wrapped non-composite type', () => {
      ...
      expect(() => assertCompositeType(GraphQLList(InputObjectType))).to.throw();
    });
  });
  ...
  describe('isWrappingType', () => {
    it('returns true for list and non-null types', () => {
      ...
      expect(() => assertWrappingType(GraphQLNonNull(ObjectType))).to.not.throw();
    });
  ...
  describe('isNullableType', () => {
    ...
    it('returns true for list of non-null types', () => {
      ...
      expect(() => assertNullableType(GraphQLList(GraphQLNonNull(ObjectType)))).to.not.throw();
    });
