...
import { GraphQLInt, GraphQLString, GraphQLBoolean } from '../scalars.js';
import { GraphQLList, GraphQLScalarType, GraphQLObjectType, GraphQLInterfaceType, GraphQLInputObjectType } from '../definition.js';
describe('Type System: Schema', () => {
  ...
  it('preserves the order of user provided types', () => {
    ...
    expect(typeNames).to.deep.equal(['Z', 'ZSub', 'Query', 'QuerySub', 'A', 'ASub', 'Boolean', 'String', '__Schema', '__Type', '__TypeKind', '__Field', '__InputValue', '__EnumValue', '__Directive', '__DirectiveLocation']); // Also check that this order is stable
  ...
  it('can be Object.toStringified', () => {
    const schema = new GraphQLSchema({});
    expect(Object.prototype.toString.call(schema)).to.equal('[object GraphQLSchema]');
  });
  describe('Validity', () => {
    describe('when not assumed valid', () => {
      ...
      it('checks the configuration for mistakes', () => {
        ...
        expect(() => new GraphQLSchema(JSON.parse)).to.throw(); // $DisableFlowOnNegativeTest

        expect(() => new GraphQLSchema({
          types: {}
        })).to.throw(); // $DisableFlowOnNegativeTest
    ...
    describe('A Schema must contain uniquely named types', () => {
      it('rejects a Schema which redefines a built-in type', () => {
        ...
        expect(() => new GraphQLSchema({
          query: QueryType
        })).to.throw('Schema must contain uniquely named types but contains multiple types named "String".');
      });
      it('rejects a Schema when a provided type has no name', () => {
        ...
        expect(() => new GraphQLSchema({
          query,
          types
        })).to.throw('One of the provided types for building the Schema is missing a name.');
      });
      it('rejects a Schema which defines an object type twice', () => {
        ...
        expect(() => new GraphQLSchema({
          types
        })).to.throw('Schema must contain uniquely named types but contains multiple types named "SameName".');
      });
