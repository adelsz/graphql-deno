...
describe('mapAsyncIterator', () => {
  ...
  it('closes source if mapper rejects', async () => {
    await testClosesSourceWithMapper(x => x > 1 ? Promise.reject(new Error('Cannot count to ' + x)) : Promise.resolve(x));
  });
  ...
  it('closes source if mapper rejects', async () => {
    await testClosesSourceWithRejectMapper(error => Promise.reject(new Error('Cannot count to ' + error.message)));
  });
});