...
import { GraphQLInt, GraphQLString, GraphQLBoolean } from '../../type/scalars.js';
import { createSourceEventStream, subscribe } from '../subscribe.js';
...
async function createSubscription(pubsub, schema = emailSchema, document = defaultSubscriptionAST) {
  const data = {
    inbox: {
      emails: [{
...
describe('Subscription Initialization Phase', () => {
  it('accepts positional arguments', async () => {
    ...
    async function* emptyAsyncIterator() {} // Empty
    // $FlowFixMe
  ...
  it('throws an error if schema is missing', async () => {
    ...
    `);
    await expectPromiseToThrow( // $DisableFlowOnNegativeTest
    () => subscribe(null, document), 'Expected null to be a GraphQL schema.');
    await expectPromiseToThrow( // $DisableFlowOnNegativeTest
    () => subscribe({
      document
  ...
  it('throws an error if document is missing', async () => {
    await expectPromiseToThrow( // $DisableFlowOnNegativeTest
    () => subscribe(emailSchema, null), 'Must provide document.');
    await expectPromiseToThrow( // $DisableFlowOnNegativeTest
    () => subscribe({
      schema: emailSchema
  ...
  it('resolves to an error for unknown subscription field', async () => {
    ...
    const {
      subscription
    } = await createSubscription(pubsub, emailSchema, ast);
  ...
  it('throws an error if subscribe does not return an iterator', async () => {
    ...
    const pubsub = new EventEmitter();
    await expectPromiseToThrow(() => createSubscription(pubsub, invalidEmailSchema), 'Subscription field must return Async Iterable. Received: "test".');
  });
  it('resolves to an error for subscription resolver errors', async () => {
    ...
    await testReportsError(subscriptionReturningErrorSchema); // Throwing an error

    const subscriptionThrowingErrorSchema = emailSchemaWithResolvers(() => {
      throw new Error('test error');
    });
    await testReportsError(subscriptionThrowingErrorSchema); // Resolving to an error

    const subscriptionResolvingErrorSchema = emailSchemaWithResolvers(() => Promise.resolve(new Error('test error')));
    await testReportsError(subscriptionResolvingErrorSchema); // Rejecting with an error

    const subscriptionRejectingErrorSchema = emailSchemaWithResolvers(() => Promise.reject(new Error('test error')));
  ...
  it('resolves to an error for source event stream resolver errors', async () => {
    ...
    async function testReportsError(schema) {
      // Promise<AsyncIterable<ExecutionResult> | ExecutionResult>
      const result = await createSourceEventStream(schema, parse(`
          subscription {
            importantEmail
          }
        `));
  ...
  it('resolves to an error if variables were wrong type', async () => {
    ...
    expect(result).to.deep.equal({
      errors: [{
        message: 'Variable "$priority" got invalid value "meow"; Int cannot represent non-integer value: "meow"',
        locations: [{
          line: 2,
          column: 21
        }]
...
describe('Subscription Publish Phase', () => {
  ...
  it('produces a payload per subscription event', async () => {
    ...
    const {
      sendImportantEmail,
      subscription
    } = await createSubscription(pubsub); // Wait for the next subscription payload.

    const payload = subscription.next(); // A new email arrives!
  ...
  it('produces a payload when there are multiple events', async () => {
    ...
    let payload = subscription.next(); // A new email arrives!
  ...
  it('should not trigger when subscription is thrown', async () => {
    ...
    payload = subscription.next(); // Throw error
    ...
    expect(caughtError).to.equal('ouch'); // A new email arrives!
